...
import aiohttp
import yaml
from plugins.discordcore import IDNotFound, GetDiscordSRVDataFailed
import logging
import sqlite3
...
class mcsmClient:
    def __init__(self, pt_url: str, pt_key: str, db_path: str):
        ...
        ----------
        pt_url : str
            MCSManager API 的 URL，如"http://example.com/api"，為與discordcore.py（先前設計出來用以支持pterodactyl的Library）相容才將前綴設為pt_。
        pt_key : str
            MCSManager API 的 api_key
        db_path : str
            SQLite 資料庫的路徑。
        """
        self.pt_url = pt_url
    ...
    async def __getServerStat(self):
        """
        取得 DiscordSRV 資料
        ...
        dict
            DiscordSRV 資料
        """
        TARGET_URL = f"{self.pt_url}/files"
        try:
            async with aiohttp.ClientSession() as session:
                ...
                # 指定內容（Body）
                data = {
                    "target": "/plugins/DiscordSRV/accounts.aof"
                }

                # 發送請求
                r = await session.post(TARGET_URL, params=params, json=data)
                # 將回應轉為JSON
                ct = await r.json()["data"]
                if ct == "":
